    ...
    <main>
        <div class="container-fluid">
            ...
            <ol class="breadcrumb mb-4">
                <li class="breadcrumb-item"><a href="{{route('penyewa.create')}}">Input Data Penyewa</a></li>
            </ol>
                <div class="card mb-4">
                    <div class="card-header"><i class="fas fa-table mr-1"></i>Data Penyewa</div>
                        ...
                        <label>Search:<input type="search" class="form-control form-control-sm" placeholder="" aria-controls="dataTable"></label>
                                ...
                                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        @php $no=0; @endphp
                                        @foreach($penyewa as $input)
                                        @php $no++; @endphp
                                        <tr>
                                            <td>{{$no}}</td>
                                            <td>{{ $input->nama_penyewa}}</td>
                                            <td>{{ $input->nik}}</td>
                                            <td>{{ $input->alamat}}</td>
                                            <td>{{ $input->no_hp}}</td>
                                            <td><img src="{{asset('upload/penyewa/' . $input->foto)}}" widht="50px" height="50px" alt="foto"></td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
